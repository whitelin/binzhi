...
class RoleController extends AdminController{
	...
	public function _initialize(){
		parent::_initialize();
		$this->_model = D('bzRole');
		$this->_node = D('bzNode');
		$this->_role_node = D("bz_role_node");
		$this->_user_role = D("bz_user_role");
	}
	...
	//删除操作
	public function del(){
		if($this->_model->delete($_GET['id']) > 0 && $this->_user_role->where(array('rid'=>array('eq',$_GET['id'])))->delete() > 0 && $this->_role_node->where(array('rid'=>array('eq',$_GET['id'])))->delete()>0){
